...
public class LobbyService implements ApplicationListener<SessionDisconnectEvent> {
    ...
    private final WordTranslateGameService wordTranslateGameService;

    private final List<PlayerDTO> lobby = new ArrayList<>();

    public LobbyService(SimpMessagingTemplate messagingTemplate, WordTranslateGameService wordTranslateGameService) {
        this.messagingTemplate = messagingTemplate;
        this.wordTranslateGameService = wordTranslateGameService;
    }
    ...
    public void startGameScheduler() {
        ...
        partitions.stream().filter(e -> e.size() == 2) //todo maybe change because we don't need to iterate through whole list.
            .forEach((players) -> {
                ...
                wordTranslateGameService.createGame(roomDTO, firstPlayer, secondPlayer);
                messagingTemplate.convertAndSendToUser(firstPlayer.getUserLogin(), "/topic/game", roomDTO);
                messagingTemplate.convertAndSendToUser(secondPlayer.getUserLogin(), "/topic/game", roomDTO);
